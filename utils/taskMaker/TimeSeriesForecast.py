...
class Task(BasicTask):
    def __init__(self, args, model_dict, data_dict):
        ...
        self.record = {
            "train": {"loss": [], "mae": [], "rmse": [], "mape": [], "mae_": [],"rmse_": []},
            "valid": {"loss": [], "mae": [], "rmse": [], "mape": [], "mae_": [],"rmse_": []},
        }
    ...
    def _mape(y_true, y_pred):
        # The data points with z-score greater than 3 accounted for about 0.27% of the normal distribution
        z_threshold = 3.0
        ape = torch.abs((y_true - y_pred) / y_true)
        # Calculate z-score
        ape_mean = torch.mean(ape)
        ape_std = torch.std(ape)
        z_scores = (ape - ape_mean) / ape_std
        filtered_ape = ape[torch.abs(z_scores) < z_threshold]
        return torch.mean(filtered_ape)*100
    ...
    def _calculate_evaluation(self, y_pred, y_true):
        ...
        rmse_inv = self._rmse(y_true,y_pred).item()
        mae_inv = self._mae(y_true, y_pred).item()
        return mae, mae_inv, rmse, rmse_inv, mape

    def test(self):
        iter_loss, iter_mae, iter_rmse, iter_mape, iter_mae_inverse,iter_rmse_inverse = [], [], [], [], [], []
        self.model.eval()
        with torch.no_grad():
            with tqdm(total=len(self.test_loader),
                      ...
                      bar_format="{desc}{bar:10}[{percentage:3.0f}% {elapsed}<{remaining}] {postfix}",
                      ) as pbar:
                for batch_idx, (batch_x, y_true) in enumerate(self.test_loader):
                    ...
                    test_loss = self.model_criterion(y_pred, y_true).item()
                    mae, mae_inverse_scale, rmse, rmse_inv, mape = self._calculate_evaluation(y_pred, y_true)
                    iter_loss.append(test_loss)
                    iter_mae.append(mae)
                    iter_rmse.append(rmse)
                    iter_mape.append(mape)
                    iter_mae_inverse.append(mae_inverse_scale)
                    iter_rmse_inverse.append(rmse_inv)
                    pbar.set_postfix_str(
                        f"{Color.R}iter loss: {Color.C}{test_loss:.4f}{Color.R}, MAE(inverse): {Color.C}{mae:.4f}({mae_inverse_scale:.4f}){Color.R}, RMSE(inverse): {Color.C}{rmse:.4f}({rmse_inv:.4f}){Color.R}, MAPE :{Color.C}{mape:.4f}%{Color.RE}",
                        refresh=True)
                    pbar.update(1)
                    #  BATCH for END
                # Record:
                ...
                epoch_rmse_inverse = np.average(iter_rmse_inverse)
                pbar.set_postfix_str(
                    f"{Color.R}Epoch[Test ]: loss:{Color.C}{epoch_loss:.4f}{Color.R}, MAE(inverse):{Color.C}{epoch_mae:.4f}({epoch_mae_inverse:.4f}){Color.R}, RMSE(inverse):{Color.C}{epoch_rmse:.4f}({epoch_rmse_inverse:.4f}){Color.R}, MAPE :{Color.C}{epoch_mape:.4f}%{Color.RE}",
                    refresh=True)
        return epoch_loss, epoch_mae, epoch_rmse, epoch_mape, epoch_mae_inverse,epoch_rmse_inverse
        ...
        with torch.no_grad():
            with tqdm(total=len(self.valid_loader),
                ...
                for batch_idx, (batch_x, y_true) in enumerate(self.valid_loader):
                    ...
                    iter_rmse_inverse.append(rmse_inv)
                    pbar.set_postfix_str(
                        f"{Color.Y}iter loss: {Color.C}{valid_loss:.4f}{Color.Y}, MAE(inverse): {Color.C}{mae:.4f}({mae_inverse_scale:.4f}){Color.Y}, RMSE(inverse): {Color.C}{rmse:.4f}({rmse_inv:.4f}){Color.Y}, MAPE :{Color.C}{mape:.4f}%{Color.RE}",
                        refresh=True)
                    pbar.update(1)
                    #  BATCH for END
                # Record:
                ...
                pbar.set_postfix_str(
                    f"{Color.Y}Epoch[Valid]: loss:{Color.C}{epoch_loss:.4f}{Color.Y}, MAE(inverse):{Color.C}{epoch_mae:.4f}({epoch_mae_inverse:.4f}){Color.Y}, RMSE(inverse):{Color.C}{epoch_rmse:.4f}({epoch_rmse_inverse:.4f}){Color.Y}, MAPE :{Color.C}{epoch_mape:.4f}%{Color.RE}",
                    refresh=True)
        return epoch_loss, epoch_mae, epoch_rmse, epoch_mape, epoch_mae_inverse, epoch_rmse_inverse
    ...
    def train(self):
        ...
        for self.epoch in range(self.args.epochs):
            ...
            with tqdm(total=len(self.train_loader),
                ...
                for batch_idx, (batch_x, y_true) in enumerate(self.train_loader):
                    ...
                    if self.args.log_interval_iter and batch_idx % self.args.log_interval_iter == 0:
                        print_log(self.log_file,
                                  f"{Color.P}epoch{self.epoch:03d}-{batch_idx:03d} {Color.RE}: loss:{loss_item:.4f}, MAE(inverse):{mae:.4f}({mae_inverse_scale:.4f}), RMSE(inverse):{rmse:.4f}({rmse_inv:.4f}), mape:{mape:.4f}")
                    # Save iter record
                    ...
                    pbar.set_postfix_str(
                        f"{Color.G}iter loss: {Color.C}{loss_item:.4f}{Color.G}, MAE(inverse): {Color.C}{mae:.4f}({mae_inverse_scale:.4f}){Color.G}, RMSE(inverse): {Color.C}{rmse:.4f}({rmse_inv:.4f}){Color.G}, MAPE :{Color.C}{mape:.4f}%{Color.RE}",
                        refresh=True)
                    pbar.update(1)
                    # BATCH for END
                # with TQDM END
                ...
                epoch_rmse_inverse = np.average(iter_rmse_inverse)
                pbar.set_postfix_str(
                    f"{Color.G}Epoch[Train]: loss:{Color.C}{epoch_loss:.4f}{Color.G}, MAE(inverse):{Color.C}{epoch_mae:.4f}({epoch_mae_inverse:.4f}){Color.G}, RMSE(inverse):{Color.C}{epoch_rmse:.4f}({epoch_rmse_inverse:.4f}){Color.G}, MAPE :{Color.C}{epoch_mape:.4f}%{Color.RE}",
                    refresh=True)
                print_log(self.log_file,
                          f"{Color.G}Train epoch-{self.epoch:03d}:{Color.RE} Train loss:{Color.C}{epoch_loss:.4f}{Color.RE}, MAE(inverse): {Color.C}{epoch_mae:.4f}({epoch_mae_inverse:.4f}){Color.RE}, RMSE: {Color.C}{epoch_rmse:.4f}{Color.RE}, MAPE:{Color.C}{epoch_mape:.4f}{Color.RE}",False)
            ...
            self.record['train']['rmse_'].append(rmse_inv)
            # VALID:
            valid_loss, valid_mae, valid_rmse, valid_mape, valid_mae_,valid_rmse_ = self.valid()
            print_log(self.log_file,
                      f"{Color.Y}Valid epoch-{self.epoch:03d}:{Color.RE} Valid loss:{Color.C}{valid_loss:.4f}{Color.RE}, MAE(inverse): {Color.C}{valid_mae:.4f}({valid_mae_:.4f}){Color.RE}, RMSE: {Color.C}{valid_rmse:.4f}{Color.RE}, MAPE:{Color.C}{valid_mape:.4f}{Color.RE}",False)
            ...
            self.record['valid']['rmse_'].append(valid_rmse_)

            torch.save(self.model.state_dict(), os.path.join(self.args.model_save_path, f"epoch-{self.epoch:03d}.pth"))
            # === END FOR: EPOCH ===

        # Test:
        test_loss, test_mae, test_rmse, test_mape, test_mae_, test_rmse_ = self.test()
        print_log(self.log_file,
                  f"{Color.R}Test :{Color.RE} Test loss:{Color.C}{test_loss:.4f}{Color.RE}, MAE(inverse): {Color.C}{test_mae:.4f}({test_mae_:.4f}){Color.RE}, RMSE: {Color.C}{test_rmse:.4f}{Color.RE}, MAPE:{Color.C}{test_mape:.4f}{Color.RE}")
        # Save Data:
        metric = pd.DataFrame({
            'epoch': epoch_list,
            ...
            'train_rmse_inverse': self.record['train']['rmse_'],
            'valid_rmse_inverse': self.record['valid']['rmse_'],
